document.addEventListener('DOMContentLoaded', function() {
    ...
    class Particle {
        ...
        draw() {
            ...
            ctx.fillStyle = `rgba(${hexToRgb(accent)}, ${this.opacity})`;
            ctx.fill();
        }
    }
        ...
        for (let a = 0; a < particles.length; a++) {
            for (let b = a + 1; b < particles.length; b++) {
                ...
                if (dist < 120) {
                    const accent = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
                    ctx.beginPath();
                    ctx.strokeStyle = `rgba(${hexToRgb(accent)}, ${0.08 * (1 - dist / 120)})`;
                    ctx.lineWidth = 0.5;
    ...
    let typingTimeout;
    let typingGreetingEl = document.getElementById('typingGreeting');
    ...
    document.querySelectorAll('.sidebar-pos-card').forEach(card => {
        card.addEventListener('click', function() {
            document.querySelectorAll('.sidebar-pos-card').forEach(c => c.classList.remove('active'));
            this.classList.add('active');
    ...
    function applyTransitions(enabled) {
        ...
        if (!enabled) {
            ...
            document.documentElement.style.setProperty('--transition', 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)');
        }
    }

    document.querySelectorAll('.toggle-switch input').forEach((toggle, index) => {
        toggle.addEventListener('change', function() {
            if (index === 0) applyTilt3D(this.checked);
    ...
    function switchPage(pageName) {
        ...
        pages.forEach(page => {
            ...
            if (page.id === pageName) {
                ...
                staggerCards(page, '.skill-card-3d, .project-card, .contact-card, .about-stat-card, .about-detail-card, .interest-item, .exp-card, .timeline-card, .skill-category-card, .skill-icon-item, .settings-section, .support-action-card, .faq-item, .support-contact-card');
            }
        });
    }

    navLinks.forEach(link => {
        link.addEventListener('click', function(e) { e.preventDefault(); switchPage(this.dataset.page); });
    });
    ...
    // CV download handled by <a download> tag
    ...
    const effects = ['effect-flip', 'effect-lift', 'effect-glow', 'effect-tilt', 'effect-rotate'];

    function applyEffects() {
        effects.forEach(cls => document.body.classList.remove(cls));
        if (state.animations.tilt3d) {
            document.body.classList.add('effect-tilt');
        }
    }

    // Patch animation toggles to also update body classes
    const origApplyTilt3D = applyTilt3D;
    applyTilt3D = function(enabled) {
        origApplyTilt3D(enabled);
        applyEffects();
    };
